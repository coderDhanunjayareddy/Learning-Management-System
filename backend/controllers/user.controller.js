...
export const getDashboardStats = async (req, res) => {
  ...
  try {
    ...
    const newSignupsResult = await pool.query(newSignupsQuery, newSignupsParams);
    ...
    const newEnrollmentsResult = await pool.query(newEnrollmentsQuery, newEnrollmentsParams);
    ...
    const activeUsersResult = await pool.query(activeUsersQuery, activeUsersParams);
    ...
    const dailySignupsResult = await pool.query(dailySignupsQuery, dailySignupsParams);
...
const enrollmentsData = labels.map(day => enrollMap[day] || 0);
    ...
    const dailyLoginsResult = await pool.query(dailyLoginsQuery, dailyLoginsParams);
...
const loginsData = labels.map(day => loginMap[day] || 0);
    ...
    const dailyLearningResult = await pool.query(dailyLearningQuery, dailyLearningParams);
