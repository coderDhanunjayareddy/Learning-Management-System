    ...
    const loadExams = async () => {
      ...
      try {
        const filterParams = {
          ...
          ...(filters.startFrom && { start_from: filters.startFrom }),
          ...(filters.startTo && { start_to: filters.startTo }),
        };

        const res = await fetchExams(filterParams, page, pageSize);
        const payload = Array.isArray(res.data)
          ? res.data
          : Array.isArray(res?.data)
            ? res.data
            : [];
        const totalCount = Number(res.total ?? res.meta?.total ?? payload.length);
