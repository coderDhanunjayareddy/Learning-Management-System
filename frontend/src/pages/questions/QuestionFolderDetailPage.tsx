...
const normalizeFolder = (item: unknown): QuestionFolder => {
  const record = item && typeof item === "object" ? (item as Record<string, unknown>) : {};
  return {
    id: record.id as string | number,
    name: typeof record.name === "string" ? record.name : "Untitled Folder",
    description: typeof record.description === "string" ? record.description : "",
    questionCount: Number(record.questionCount ?? record.question_count ?? 0),
  };
};
...
export default function QuestionFolderDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadFolder = async () => {
      ...
      try {
        ...
        setFolder(null);
        setError(extractApiErrorMessage(err, "Failed to load folder"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteAllQuestions = async () => {
    ...
    try {
      ...
      setError(extractApiErrorMessage(err, "Failed to delete folder questions"));
    } finally {
      setDeleting(false);
    }
  };
